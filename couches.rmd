...
```{r include=FALSE}
packages<- c("dplyr", "stringr", "readr", "stargazer", "knitr", "DataExplorer", "ggplot2", "car","ggExtra", "psych", "PerformanceAnalytics", "corrplot", "mice", "plotly", "geojsonio", "rjson", "geojsonR", "pls", "fastDummies", "Metrics", "leaps", "glmnet", "glmnetUtils") #add new libraries here
...
```
# Backward Stepwise Regression using Leaps Package
```{r}
...
#lasso_cv_sin= cv.glmnet(as.matrix(list_after_2019.sin_clean[,-46]),list_after_2019.sin_clean[,46],family="gaussian",alpha=1, nfolds=10)
#lasso_cv_hkg= cv.glmnet(as.matrix(list_after_2019.hkg_clean[,-46]),list_after_2019.hkg_clean[,46],family="gaussian",alpha=1, nfolds=10)
#lasso_cv_nrt= cv.glmnet(as.matrix(list_after_2019.nrt_clean[,-46]),list_after_2019.nrt_clean[,46],family="gaussian",alpha=1, nfolds=10)
#lasso_cv_tpe= cv.glmnet(as.matrix(list_after_2019.tpe_clean[,-46]),list_after_2019.tpe_clean[,46],family="gaussian",alpha=1, nfolds=10)
...
lasso_coef_top5.sin <- returnTopBtmCoefs(lasso_coef_normalised.sin, 10)
paintGraphCoefs(lasso_coef_top5.sin, 10, "Singapore")
lasso_coef_top5.nrt <- returnTopBtmCoefs(lasso_coef_normalised.nrt, 10)
paintGraphCoefs(lasso_coef_top5.nrt, 10, "Tokyo")
lasso_coef_top5.tpe <- returnTopBtmCoefs(lasso_coef_normalised.tpe, 10)
paintGraphCoefs(lasso_coef_top5.tpe, 10, "Taipei")
lasso_coef_top5.hkg <- returnTopBtmCoefs(lasso_coef_normalised.hkg, 10)
paintGraphCoefs(lasso_coef_top5.hkg, 10, "Hong Kong")

lasso_coef_btm5.sin <- returnTopBtmCoefs(lasso_coef_normalised.sin, -10)
paintGraphCoefs(lasso_coef_btm5.sin, -10, "Singapore")
lasso_coef_btm5.nrt <- returnTopBtmCoefs(lasso_coef_normalised.nrt, -10)
paintGraphCoefs(lasso_coef_btm5.sin, -10, "Tokyo")
lasso_coef_btm5.tpe <- returnTopBtmCoefs(lasso_coef_normalised.tpe, -10)
paintGraphCoefs(lasso_coef_btm5.tpe, -10, "Taipei")
lasso_coef_btm5.hkg <- returnTopBtmCoefs(lasso_coef_normalised.hkg, -10)
paintGraphCoefs(lasso_coef_btm5.hkg, -10, "Hong Kong")
```
...
Comments from Kevin: Calculated the MAE value, but this number is probably more useful if its a comparison between mo dels.
